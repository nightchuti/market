...
function TradeProductBar({ room, getImgUrl, navigate }) {
  ...
  return (
    <div className="cp-trade-pbar">
      <div className="cp-trade-pbar-item" onClick={() => wantId && navigate(`/products/${wantId}`)} style={{ cursor: wantId ? "pointer" : "default" }}>
        ...
        <div className="cp-trade-pbar-info">
          ...
          {wantPrice != null && <span className="cp-trade-pbar-price">฿{Number(wantPrice).toLocaleString()}</span>}
        </div>
      </div>
      <div className="cp-trade-pbar-arrow">⇄</div>
      <div className="cp-trade-pbar-item" onClick={() => offerId && navigate(`/products/${offerId}`)} style={{ cursor: offerId ? "pointer" : "default" }}>
        ...
        <div className="cp-trade-pbar-info">
          ...
          {offerPrice != null && <span className="cp-trade-pbar-price">฿{Number(offerPrice).toLocaleString()}</span>}
        </div>
      </div>
    </div>
  );
...
function TradeMessageCard({ msg, myId, getImgUrl }) {
  ...
  return (
    <div className="cp-trade-msgcard" style={{ background: cfg.bg, border: `1px solid ${cfg.border}` }}>
      ...
      {msg.text && msg.messageType !== "trade_request" && <p className="cp-trade-msgcard-text">{msg.text}</p>}
      {msg.text && msg.messageType === "trade_request" && !meta.offeredProduct && <p className="cp-trade-msgcard-text">{msg.text}</p>}
    </div>
  );
}

// ── Confirm Swap Modal ────────────────────────────────────
function ConfirmSwapModal({ room, onConfirm, onClose, getImgUrl, navigate, myId }) {
  const [step, setStep] = useState("choose");
  const [confirming, setConfirming] = useState(false);

  // ── ตรวจ role ──
  // Owner (B) = เจ้าของ productId → จะได้รับ offeredProductId จาก A
  // Requester (A) = เจ้าของ offeredProductId → จะได้รับ productId จาก B
  const ownerIdOfProduct = String(room?.productId?.user?._id || room?.productId?.user || "");
  const iAmOwner = ownerIdOfProduct === String(myId);

  // สินค้าที่เราจะได้รับ (ของอีกฝ่าย) → ใช้ใน checkout เพื่อจ่ายค่าส่ง
  const productToReceive = iAmOwner ? room?.offeredProductId : room?.productId;
  const productToReceiveId = typeof productToReceive === "object" ? productToReceive?._id : productToReceive;

  const handleGoToCheckout = () => {
    if (!productToReceiveId) return alert("ไม่พบข้อมูลสินค้า");
    navigate("/checkout", {
      state: {
        items: [{
          _id: productToReceiveId,
          product: productToReceiveId,
          title: productToReceive?.title || "สินค้าแลกเปลี่ยน",
          price: 0,
          images: productToReceive?.images || [],
          qty: 1,
          deliveryType: productToReceive?.deliveryType || "both",
          isTrade: true,
          roomId: room?._id,
        }],
        isTrade: true,
    ...
    return (
      <div className="cp-modal-overlay">
        <div className="cp-modal">
          ...
          {/* แสดงสินค้าที่จะได้รับ */}
          {productToReceive && (
            <div style={{ background: "#f8fafc", borderRadius: 8, padding: "10px 12px", marginBottom: 12, display: "flex", gap: 10, alignItems: "center" }}>
              <img src={getImgUrl(productToReceive?.images?.[0])} alt="" style={{ width: 48, height: 48, objectFit: "cover", borderRadius: 6 }} />
              <div>
                <p style={{ margin: 0, fontSize: 13, fontWeight: 600 }}>{productToReceive?.title}</p>
                <p style={{ margin: 0, fontSize: 12, color: "#6b7280" }}>สินค้าที่คุณจะได้รับ</p>
              </div>
            </div>
          )}
          ...
          <div className="cp-modal-btns">
            ...
            <button className="cp-btn accept" onClick={handleGoToCheckout}>
              💳 จัดส่งผ่านระบบ
            </button>
          </div>
      ...
      <div className="cp-modal-overlay">
        <div className="cp-modal">
          ...
          <div className="cp-modal-btns">
            ...
            <button className="cp-btn accept" disabled={confirming}
              onClick={async () => { setConfirming(true); await onConfirm("meetup"); setConfirming(false); }}>
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const socket = io(API_URL, { auth: { token }, transports: ["polling", "websocket"], reconnection: true, reconnectionAttempts: 5, reconnectionDelay: 1000 });
    ...
    socket.on("receive_trade_message", (msg) => {
      setMessages(prev => prev.find(m => String(m._id) === String(msg._id)) ? prev : [...prev, msg]);
      const statusMap = { trade_accept: "accepted", trade_reject: "rejected", trade_cancel: "cancelled", trade_confirm: "completed" };
      if (statusMap[msg.messageType]) setRoom(prev => prev ? { ...prev, tradeStatus: statusMap[msg.messageType] } : prev);
    });
  ...
  const handleInput = (e) => {
    ...
    if (socketRef.current?.connected) {
      ...
      typingTimer.current = setTimeout(() => socketRef.current?.emit("stop_typing", { roomId, userId: myId }), 1500);
    }
  };
  ...
  const tradeAction = async (action) => {
    ...
    try {
      ...
      socketRef.current?.emit("send_trade_message", { roomId, sender: myId, messageType: `trade_${action}`, metadata: {} });
      fetchRoom();
    } catch (err) {
  ...
  const handleConfirmSwap = async (method) => {
    const headers = { Authorization: `Bearer ${token}` };
    try {
      const res = await axios.put(`${API_URL}/api/trades/${roomId}/confirm-chat`, {}, { headers });
      socketRef.current?.emit("send_trade_message", { roomId, sender: myId, messageType: "trade_confirm", metadata: { method } });
      if (res.data.completed) {
        socketRef.current?.emit("trade_status_update", { roomId, status: "completed", updatedBy: myId });
        setShowSwapModal(false);
        setRoom(prev => prev ? { ...prev, tradeStatus: "completed" } : prev); // ← ปิดห้องทันที ไม่ต้อง fetch
      } else {
        socketRef.current?.emit("trade_status_update", { roomId, status: "accepted", updatedBy: myId });
        setShowSwapModal(false);
        alert(`✅ รับทราบแล้ว! รอให้อีกฝ่ายยืนยันรับสินค้าด้วย (${res.data.confirmedCount}/2)`); // ← แจ้งว่ารออีกฝ่าย
      }
    } catch (err) {
      alert(err.response?.data?.error || "เกิดข้อผิดพลาด");
      setShowSwapModal(false);
  ...
  return (
    <div className="cp-wrap">
      <header className="cp-header">
        ...
        <img className="cp-havatar" src={getAvatar(otherUser)} alt="" onClick={() => navigate(`/profile/${otherUser?._id}`)} style={{ cursor: "pointer" }} />
        ...
        <>
          ...
          {isClosed() && room.tradeStatus && (
            <div className="cp-trade-closed-status" style={{ borderColor: STATUS[room.tradeStatus]?.c }}>
              <span style={{ color: STATUS[room.tradeStatus]?.c }}>{STATUS[room.tradeStatus]?.t}</span>
            </div>
          )}
        </>
      )}
      ...
      {room?.type === "trade" && !isClosed() && (
        <div className="cp-trade">
          {room.tradeStatus && (
            <p className="cp-tstatus" style={{ color: STATUS[room.tradeStatus]?.c }}>{STATUS[room.tradeStatus]?.t}</p>
          )}
          {["pending", "negotiating"].includes(room?.tradeStatus) && (
            <div className="cp-tbtns">
              {isOwner() ? (
                ...
                <div className="cp-pending-bar">⏳ รอเจ้าของสินค้าตอบรับคำขอเทรด...</div>
              )}
              <button className="cp-btn cancel" onClick={() => window.confirm("ยืนยันยกเลิก?") && tradeAction("cancel")}>🚫 ยกเลิก</button>
            </div>
          )}
          {room?.tradeStatus === "accepted" && (
            <div className="cp-tbtns">
              <button className="cp-btn accept" onClick={() => setShowSwapModal(true)}>🎉 ยืนยันส่ง / รับสินค้าแล้ว</button>
              <button className="cp-btn cancel" onClick={() => window.confirm("ยืนยันยกเลิก?") && tradeAction("cancel")}>🚫 ยกเลิก</button>
            </div>
          )}
        </div>
      ...
      {room?.type === "normal" && room?.productId && (
        <div className="cp-pbar" onClick={() => navigate(`/products/${room.productId._id || room.productId}`)} style={{ cursor: "pointer" }}>
      ...
      <main className="cp-msgs">
        {messages.length === 0 && <p className="cp-empty">ยังไม่มีข้อความ — เริ่มสนทนาได้เลย 👋</p>}
        {messages.map((msg, i) => {
